...
theme download assets/deploy_sh.txt || true
LAST_deploy_sh=$(cat "$INPUT_PATH/assets/deploy_sh.txt")

echo "---> LAST_deploy_sh is $LAST_deploy_sh"

# Check if we could retrieve a deploy SHA.
if [ ! -z "$LAST_deploy_sh" ]; then
  # Get a list of files that have changed in our Shopify theme since the last deploy.
  CHANGED_FILES=$(git diff "$LAST_deploy_sh..$GITHUB_SHA" --name-only -- $INPUT_PATH/assets $INPUT_PATH/config $INPUT_PATH/layout $INPUT_PATH/locales $INPUT_PATH/sections $INPUT_PATH/snippets $INPUT_PATH/templates)
...
echo $GITHUB_SHA > "$INPUT_PATH/assets/deploy_sh.txt"
theme deploy assets/deploy_sh.txt